...
This audit was read-only. It did not change code, infrastructure, databases,
deployments, settings, DNS, or secrets.
...
- no `litestream.yml`;
- no Litestream binary in the Docker image;
- no Litestream-supervised runtime command;
- no object/rclone backup job for session artifacts;
- no automated restore verification.

The deployment guide presents Litestream and rclone as optional examples, not
active configuration. Its Railway Litestream command also omits
`--sessions-dir /data/sessions`; applying it literally would cause the
application to fall back to a path outside the intended mount. It must not be
...
1. Create a SQLite-consistent backup using SQLite's backup API or a verified WAL-aware tool.
2. Independently copy the retained session artifacts and a checksum manifest.
3. Restore both into an isolated scratch environment.
4. Run `PRAGMA integrity_check`, reconcile table counts and purchase/entitlement ledgers, and open representative reports/media.
5. Record the measured recovery point objective (RPO), recovery time objective (RTO), restore date, and operator.
6. Keep an encrypted off-platform copy in addition to any Railway snapshots.
